        ...
        <main className="bg-creme dark:bg-midnight min-h-screen transition-colors duration-500">
            ...
            <section className="relative py-32 flex items-center justify-center">
                ...
                <div className="relative z-10 text-center max-w-4xl px-6">
                    ...
                    <h1 className="text-5xl md:text-7xl font-serif font-bold text-espresso dark:text-ivory mb-6">FAQs</h1>
                    <p className="text-xl text-espresso-light dark:text-ivory/80 max-w-2xl mx-auto mb-8">
                        Everything you need to know about Amrit Milk, our farm, and your subscription.
                    </p>

                    {/* Search */}
                    <div className="max-w-xl mx-auto relative">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-espresso-muted dark:text-ivory/60" />
                        <input
                            type="text"
                            placeholder="Search for answers..."
                            className="w-full pl-12 pr-4 py-4 bg-white dark:bg-midnight-mid border border-creme-dark dark:border-white/10 rounded-2xl text-espresso dark:text-ivory placeholder:text-espresso-muted dark:placeholder:text-ivory/60 focus:border-gold focus:outline-none transition-colors"
                            value={searchQuery}
            ...
            {searchQuery && (
                <Section className="pt-0">
                    <div className="max-w-4xl mx-auto">
                        <p className="text-espresso-muted dark:text-ivory/60 mb-6">
                            Found {filteredFaqs.length} result{filteredFaqs.length !== 1 ? 's' : ''} for "{searchQuery}"
                        </p>
                        {filteredFaqs.length > 0 ? (
                            <div className="space-y-4">
                                {filteredFaqs.map((faq) => (
                                    <div key={faq.key} className="bg-white dark:bg-midnight-mid border border-creme-dark dark:border-white/5 p-6 rounded-2xl shadow-soft dark:shadow-card-dark">
                                        <h3 className="text-lg font-bold text-espresso dark:text-ivory mb-3">{faq.question}</h3>
                                        <p className="text-espresso-light dark:text-ivory/80">{faq.answer}</p>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <p className="text-espresso-muted dark:text-ivory/60 text-center py-12">No results found. Try a different search term.</p>
                        )}
                    </div>
                </Section>
            )}
            ...
            {!searchQuery && (
                <>
                    <section className="border-y border-creme-dark dark:border-white/10 bg-creme-light dark:bg-midnight-mid sticky top-20 z-30">
                                ...
                                {faqCategories.map((cat, index) => (
                                    <button
                                        ...
                                        className={`px-6 py-2 rounded-full whitespace-nowrap transition-all ${activeCategory === index
                                            ...
                                            : 'bg-white dark:bg-midnight text-espresso-light dark:text-ivory/80 hover:text-espresso dark:hover:text-ivory border border-creme-dark dark:border-white/10'
                                            }`}
                                    >
                                        {cat.name}
                                    </button>
                    ...
                    <Section>
                        <div className="max-w-4xl mx-auto">
                            <h2 className="text-2xl font-bold text-espresso dark:text-ivory mb-8">{faqCategories[activeCategory].name}</h2>
                            <div className="space-y-4">
                                {faqCategories[activeCategory].faqs.map((faq, index) => {
                                    const key = `${activeCategory}-${index}`;
                                    const isOpen = openItems[key];
                                    return (
                                        <div key={key} className="bg-white dark:bg-midnight-mid border border-creme-dark dark:border-white/5 rounded-2xl overflow-hidden shadow-soft dark:shadow-card-dark">
                                            <button
                                                className="w-full px-6 py-5 flex items-center justify-between text-left"
                                                onClick={() => toggleItem(key)}
                                            >
                                                <span className="text-lg font-medium text-espresso dark:text-ivory pr-4">{faq.question}</span>
                                            ...
                                            <div className={`overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                                                <p className="px-6 pb-5 text-espresso-light dark:text-ivory/80 leading-relaxed">{faq.answer}</p>
                                            </div>
                                        </div>
                                    );
                                })}
            ...
            <Section className="bg-creme-light dark:bg-midnight-mid text-center">
                <h2 className="text-3xl font-serif font-bold text-espresso dark:text-ivory mb-6">Still Have Questions?</h2>
                <p className="text-espresso-muted dark:text-ivory/60 max-w-xl mx-auto mb-8">

...
import { ArrowRight, Award, Heart, MessageCircle, ShieldCheck, Star, Truck, Zap } from "lucide-react";
...
export default async function ProductPage({ params }: { params: { slug: string } }) {
    ...
    const bestSellingProducts = products
        .filter((p: any) => p.badge?.toLowerCase().includes('seller') || p.badge?.toLowerCase().includes('value') || p.featured)
        .slice(0, 4);
            ...
            <div className="space-y-0">
                ...
                <Section className="relative overflow-hidden">
                    <div className="max-w-4xl mx-auto text-center">
                        <span className="text-theme-accent font-bold uppercase tracking-[0.3em] text-sm mb-4 block">The Product</span>
                        <h2 className="text-4xl md:text-5xl font-serif font-bold text-theme-primary mb-8">What it is</h2>
                ...
                <Section className="bg-theme-secondary/30">
                     <div className="max-w-5xl mx-auto px-6">
                        <div className="text-center mb-16">
                            <span className="text-theme-accent font-bold uppercase tracking-[0.3em] text-sm mb-4 block">Safety First</span>
                            <h2 className="text-4xl font-serif font-bold text-theme-primary">Why it's safe for families</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {content.whySafe.map((item: string, i: number) => ( // Explicitly typed as string
                                <div key={i} className="flex items-center gap-4 p-6 rounded-2xl bg-white dark:bg-white/5 border border-theme-light">
                                    <ShieldCheck className="w-8 h-8 text-terracotta dark:text-gold" />
                                    <p className="text-lg font-medium text-theme-primary">{item}</p>
                                </div>
                            ))}
                        </div>
                     </div>
                ...
                <Section>
                     <div className="max-w-5xl mx-auto px-6">
                        <div className="text-center mb-16">
                            <span className="text-theme-accent font-bold uppercase tracking-[0.3em] text-sm mb-4 block">The Amrit Standard</span>
                            <h2 className="text-4xl font-serif font-bold text-theme-primary">What makes Amrit Milk different</h2>
                        </div>
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {content.difference.map((item: string, i: number) => ( // Typed
                                <div key={i} className="flex items-start gap-4 p-6 rounded-2xl bg-theme-elevated border border-terracotta/20 dark:border-gold/20">
                                    <div className="w-10 h-10 rounded-full bg-terracotta text-white flex items-center justify-center flex-shrink-0">
                                        <Award className="w-5 h-5" />
                                    </div>
                                    <p className="text-lg font-medium text-theme-primary pt-2">{item}</p>
                                </div>
                            ))}
                        </div>
                     </div>
                ...
                <Section className="bg-theme-primary relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-terracotta/5 dark:bg-gold/5"></div>
                     <div className="max-w-5xl mx-auto px-6 relative z-10">
                        <div className="text-center mb-16">
                            <span className="text-theme-accent font-bold uppercase tracking-[0.3em] text-sm mb-4 block">Ideal For</span>
                ...
                <Section>
                    <div className="max-w-4xl mx-auto px-6 text-center">
                         <span className="text-theme-accent font-bold uppercase tracking-[0.3em] text-sm mb-4 block">Service</span>
                         <h2 className="text-4xl font-serif font-bold text-theme-primary mb-12">Ordering & Delivery</h2>
                         
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                             {content.ordering.map((item: string, i: number) => ( // Typed
                ...
                <Section className="bg-theme-secondary/30">
                    <div className="max-w-7xl mx-auto px-6">
                        <div className="flex justify-between items-end mb-12">
                            <div>
                                <span className="text-theme-accent font-bold uppercase tracking-[0.3em] text-sm mb-4 block">Popular Choices</span>
